...
class UAVPTZEnvWindow(BaseEnvWindow):
    """Window manager for the UAV environment."""

    def __init__(self, env: UAVPTZEnv, window_name: str = "IsaacLab"):
...
@configclass
class UAVPTZEnvCfg(DirectRLEnvCfg):
    # env
    ...
    ui_window_class_type = UAVPTZEnvWindow
...
class UAVPTZEnv(DirectRLEnv):
    cfg: UAVPTZEnvCfg

    def __init__(self, cfg: UAVPTZEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self._ptz_action[:, 0, :] = self.cfg.ptz_scale * self._actions[:, 4:7]

    def _apply_action(self):
        self._robot.set_external_force_and_torque(self._thrust, self._moment, body_ids=self._body_id)
        self._robot.set_joint_effort_target(self._ptz_action[:, :, 0], joint_ids=self.ptz_joint_x_idx)
        self._robot.set_joint_effort_target(self._ptz_action[:, :, 1], joint_ids=self.ptz_joint_y_idx)
        self._robot.set_joint_effort_target(self._ptz_action[:, :, 2], joint_ids=self.ptz_joint_z_idx)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._desired_pos_w[env_ids, 2] = torch.zeros_like(self._desired_pos_w[env_ids, 2]).uniform_(1.0, 9.0)
        # Reset robot state
