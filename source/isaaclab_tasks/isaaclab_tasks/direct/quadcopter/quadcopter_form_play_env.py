...
@configclass
class QuadcopterFormPlayEnvCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=4, env_spacing=22.0, replicate_physics=True) # 65536 32768 16384 
    # scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=4096, env_spacing=2.5, replicate_physics=True) 
    # 8192
    # robot
    ...
    max_formation_width = 5.0 

    angle_reward_scale = 1.0
    distance_to_goal_reward_scale = 150.0
    goal_reached_reward_scale = 10.0
    effort_reward_scale = 1.0
    speed_reward_scale = 0.1
    formation_reward_scale = 1.0

class QuadcopterFormPlayEnv(DirectRLEnv):
    cfg: QuadcopterFormPlayEnvCfg

    def __init__(self, cfg: QuadcopterFormPlayEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def formation_check(self, data_or_result = True):        
        ...
        if data_or_result:
            ...
            dones = (f1 > self.cfg.max_formation_width) + (f2 > self.cfg.max_formation_width) + (f3 > self.cfg.max_formation_width) + (f4 > self.cfg.max_formation_width)
