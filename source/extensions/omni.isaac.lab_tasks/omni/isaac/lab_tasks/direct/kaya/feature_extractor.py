...
class FeatureExtractor:
    ...
    def _preprocess_depth_image(self, depth_img: torch.Tensor) -> torch.Tensor:
        """Preprocesses the input images.
        Args:
            depth_img (torch.Tensor): Depth image tensor. Shape: (N, H, W, 1).
        Returns:
            torch.Tensor: Preprocessed depth
        """
        depth_img[depth_img == float("inf")] = 0
        depth_img /= 5.0
        depth_img /= torch.max(depth_img)
    ...
    def depth_step(self, depth_img: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Extracts the features using the images and trains the model if the train flag is set to True.
        ...
        depth_img = self._preprocess_depth_image(depth_img)
