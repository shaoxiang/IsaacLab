...
class TennisSceneCfg(InteractiveSceneCfg):
    ...
    tennis_ball_rigid: RigidObjectCfg = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(-0.1, 0.0, 0.7)),
    )
    ...
    contact_sensor: ContactSensorCfg = ContactSensorCfg(
        prim_path="/World/envs/env_.*/Robot/check_goal", update_period=0.01, history_length=3, debug_vis=False
    )
...
class KayaTennisEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.cat(
            [
                ...
                desired_pos_b,
                self._tennis_ball_rigid.data.root_vel_w,
            ],
            dim=-1,
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        reset_ball = self._tennis_ball_rigid.data.root_pos_w[:, 2] < 0.05
    ...
    def reset_tennis_ball_rigid(self, env_ids: torch.Tensor | None):
        ...
        default_tennis_ball_rigid_state[:, :2] = torch.zeros_like(self._tennis_ball_pos[env_ids, :2]).uniform_(-1.0, 1.0)
        default_tennis_ball_rigid_state[:, 2] = torch.zeros_like(self._tennis_ball_pos[env_ids, 2]).uniform_(2.0, 5.0)
